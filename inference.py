...
# Load model
cv = pickle.load(open('Sentiment_LSTM/cv_model.sav', 'rb'))
model_load = torch.load('Sentiment_LSTM/LSTM_sentiment_model.pth')
...
def inference(text_input):
    ...
    toc = datetime.now()
    if cls.item() == 0:
        return f'Negative | p = {out.max()} | Time = {(toc - tic).total_seconds()} s'
    else:
        return f'Positive/Neutral | p = {out.max()} | Time = {(toc - tic).total_seconds()} s'
